...
public interface OptionMarketAPI {
    ...
    //公共-获取标的指数
    @GET("/api/option/v5/underlying")
    Call<JSONArray> getUnderlying();

    //公共-获取期权合约
    @GET("/api/option/v5/instruments/{underlying}")
    ...
    //公共-获取某个Ticker信息
    @GET("/api/option/v5/instruments/{instrument_id}/ticker")
    Call<JSONObject> getTicker(@Path("instrument_id") String instrument_id);

    //公共-获取K线数据
    @GET("/api/option/v5/instruments/{instrument_id}/candles")
    ...
    //公共-获取历史结算/行权记录
    @GET("/api/option/v5/settlement/history/{underlying}")
