...
public interface OptionMarketAPI {
    ...
    //公共-获取标的指数
    @GET("/api/v5/option/underlying")
    Call<JSONArray> getUnderlying();

    //公共-获取期权合约
    @GET("/api/v5/option/instruments/{underlying}")
    ...
    //公共-获取某个Ticker信息
    @GET("/api/v5/option/instruments/{instrument_id}/ticker")
    Call<JSONObject> getTicker(@Path("instrument_id") String instrument_id);

    //公共-获取K线数据
    @GET("/api/v5/option/instruments/{instrument_id}/candles")
    ...
    //公共-获取历史结算/行权记录
    @GET("/api/v5/option/settlement/history/{underlying}")
